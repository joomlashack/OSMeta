...
class VM_ProductMetatagsContainer extends MetatagsContainer
{
	public function getMetatags($lim0, $lim, $filter = null)
	{
		...
		if ($category_id > 0)
		{
			$sql .= " AND EXISTS (SELECT 1 FROM #__vm_product_category_xref WHERE #__vm_product_category_xref.category_id=" .
				$db->quote($category_id) . " AND #__vm_product_category_xref.product_id=c.product_id)";
	 ...
	 * @param   array  $disabledFields       Disabled fields
	 *
	 * @return  array;
	 */
