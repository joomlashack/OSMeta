...
class VM2_ProductMetatagsContainer extends MetatagsContainer
{
	...
	public function getMetatags($lim0, $lim, $filter = null)
	{
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS
			...
			LEFT JOIN
			#__osmeta_metadata m ON m.item_id=c.virtuemart_product_id and m.item_type={$this->code}
			WHERE 1 ";
	...
	public function getPages($lim0, $lim, $filter = null)
	{
		...
		if ($category_id > 0)
		{
			$sql .= " AND EXISTS (SELECT 1 FROM #__virtuemart_product_categories WHERE #__virtuemart_product_categories.virtuemart_category_id=" .
				$db->quote($category_id) . " AND #__virtuemart_product_categories.virtuemart_product_id=c.virtuemart_product_id)";
