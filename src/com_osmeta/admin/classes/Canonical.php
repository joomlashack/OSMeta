...
class OSMetaCanonicalURL
{
    ...
    {
        $db = JFactory::getDBO();
        $db->setQuery("SELECT canonical_url, action FROM `#__osmeta_canonical_url` WHERE url="
            . $db->quote($originalURL));
    ...
    {
        $db = JFactory::getDBO();
        $db->setQuery("SELECT id, url, canonical_url, action FROM `#__osmeta_canonical_url` WHERE id="
            . $db->quote($id));
