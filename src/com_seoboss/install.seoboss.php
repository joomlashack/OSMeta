...
if (!function_exists('jb_com_install'))
{
	function jb_com_install()
	{
		...
		if (!$db->loadAssoc())
		{
			$db->setQuery('CREATE TABLE  IF NOT EXISTS `#__seoboss_canonical_url` (
`id` INT NOT NULL AUTO_INCREMENT ,
`url` VARCHAR(255) NOT NULL ,
...
');
		...
		$db->query();
		$db->setQuery("UPDATE #__seoboss_settings SET joomboss_registration_code='25ce8bfd8e4c2dec4effc6d92b9eb152'");
		$db->query();
